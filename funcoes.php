...
function miniatura() {
	...
	if (count($arg_list) > 0) {
		$lnk_img = $arg_list[0];
		$tam_img = getimagesize($lnk_img);
		$alt_img = $tam_img[0];
		$lar_img = $tam_img[1];
		
		$alt_porcentagem = 300 / ($alt_img / 100);
		$lar_porcentagem = 300 / ($lar_img / 100);
		if ($alt_img > $lar_img) {
			$lar_porcentagem = $alt_porcentagem;
		} else {
			$alt_porcentagem = $lar_porcentagem;
		}
		$alt_final = intval(($alt_img / 100) * $alt_porcentagem);
		$lar_final = intval(($lar_img / 100) * $lar_porcentagem);
	}
	return array($alt_final,$lar_final);
}
function tamanho_maximo() {
	$arg_list = func_get_args();
	if (count($arg_list) > 0) {
		$lnk_img = $arg_list[0];
		$tam_img = getimagesize($lnk_img);
		...
		$alt_porcentagem = 768 / ($alt_img / 100);
		$lar_porcentagem = 1024 / ($lar_img / 100);
		if ($alt_img > $lar_img) {
			$lar_porcentagem = $alt_porcentagem;
		} else {
			$alt_porcentagem = $lar_porcentagem;
...
function icone() {
	...
	if (count($arg_list) > 0) {
		...
		$alt_porcentagem = 50 / ($alt_img / 100);
		$lar_porcentagem = 50 / ($lar_img / 100);
		if ($alt_img > $lar_img) {
			$lar_porcentagem = $alt_porcentagem;
		} else {
			$alt_porcentagem = $lar_porcentagem;
